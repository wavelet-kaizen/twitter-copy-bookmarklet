...
export class TokenExtractor {
   ...
   */
  static extractTokensFromCookies(documentRef: Document = document): Partial<TwitterTokens> {
    const tokens: Partial<TwitterTokens> = {};
    
    const csrfToken = this.getTokenFromCookie('ct0', documentRef);
    const guestToken = this.getTokenFromCookie('gt', documentRef);
    const twitterId = this.getTokenFromCookie('twid', documentRef);
   ...
   */
  static extractBearerToken(pageWindow: Window = window): string {
    try {
   ...
   * 絵文字の正規表現パターンを抽出
   */
  static extractEmojiRegexp(pageWindow: Window = window): RegExp {
    try {
   ...
   * スクリプトURLの取得
   */
  static getScriptURL(scriptPattern: string, documentRef: Document = document): string {
    const regex = new RegExp(`${scriptPattern}\\.[\\w-]*\\.js`);
    const scripts = Array.from(documentRef.querySelectorAll<HTMLScriptElement>('script'));
    const targetScript = scripts.find(script => regex.test(script.src));
    return targetScript?.src || '';
  }

  private static getTokenFromCookie(name: string, documentRef: Document): string | undefined {
    const match = documentRef.cookie.match(new RegExp(`${name}=([^;]*);?`));
    return match?.[1];
  }
  ...
  private static getModuleParameter(
    ...
    try {
      const candidateModules = this.getCandidateModules(moduleName, pageWindow);
  ...
  private static getCandidateModules(moduleName: string | undefined, pageWindow: Window): WebpackChunkEntry[] {
    const rawChunks = pageWindow.webpackChunk_twitter_responsive_web as unknown[] | undefined;
   ...
   */
  static extractAudioSpaceQueryId(pageWindow: Window = window): string {
