...
export type NgLevel = 0 | 1 | 2 | 3;

export type ClipboardWriter = (text: string) => Promise<void>;

export interface TwitterCopyRuntimeOptions {
  pageWindow?: Window;
  document?: Document;
  location?: Location;
  writeClipboard?: ClipboardWriter;
  loadingManager?: Pick<typeof LoadingManager, 'stopLoading'>;
}
...
export class TwitterCopyBookmarklet {
  ...
  private runtime: Required<TwitterCopyRuntimeOptions>;

  constructor(avoidNgLevel: NgLevel = 0, runtimeOptions: TwitterCopyRuntimeOptions = {}) {
    this.runtime = this.createRuntime(runtimeOptions);
    const settings = createSettings(avoidNgLevel);
    const domain = this.runtime.location.hostname;
    ...
    // 認証トークンの取得
    const cookieTokens = TokenExtractor.extractTokensFromCookies(this.runtime.document);
    const bearerToken = TokenExtractor.extractBearerToken(this.runtime.pageWindow);
  ...
  async copyTweet(): Promise<string> {
    const tweetId = TokenExtractor.extractTweetId(this.runtime.location);
    if (!tweetId) {
    ...
    try {
      ...
      // クリップボードにコピー
      await this.runtime.writeClipboard(formattedTweet);
  ...
  private createRuntime(runtimeOptions: TwitterCopyRuntimeOptions): Required<TwitterCopyRuntimeOptions> {
    const pageWindow = runtimeOptions.pageWindow ?? window;
    const documentRef = runtimeOptions.document ?? pageWindow.document;
    const locationRef = runtimeOptions.location ?? pageWindow.location;
    const writeClipboard = runtimeOptions.writeClipboard
    ...
    return {
      ...
      writeClipboard,
      loadingManager: runtimeOptions.loadingManager ?? LoadingManager,
    };
  }
  ...
  private async processAudioSpaces(rootTweet: TweetData): Promise<void> {
    ...
    const queryId = TokenExtractor.extractAudioSpaceQueryId(this.runtime.pageWindow);
    if (!queryId) {
      return;
    }
  ...
  private formatTweet(tweetData: TweetData): string {
    const emojiRegExp = TokenExtractor.extractEmojiRegexp(this.runtime.pageWindow);
